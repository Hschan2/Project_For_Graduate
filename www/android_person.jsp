...
<%	
	...
	String user_area =new String(area.getBytes("8859_1"),"utf-8");
	...
	try{
		//test request link
		//http://localhost:8080/android_test02/android_person.jsp?id=1&title=1&emp=%EA%B3%84%EC%95%BD%EC%A7%81&gender=%EB%82%A8%EC%9E%90&area=%EC%9D%B8%EC%B2%9C%20|%20%EC%B6%A9%EB%82%A8%20|&week=%EB%AA%A9%EC%9A%94%EC%9D%BC%20|&strong=%EB%A7%90%EC%A3%BC%EB%B3%80%EC%9D%B4%20%EC%A2%8B%EC%9D%8C%20|%20%EC%9A%94%EB%A6%AC%EC%8B%A4%EB%A0%A5%20%EC%9A%B0%EC%88%98%20|&foreign=%EC%98%81%EC%96%B4%20%EC%A4%91%20|&money=2133&spinner=1
		...
		for(int i =1; i<=LIMIT;i++){
			...
			String Sql_all = "SELECT user_number, count(case when hire=(?) then 1 end)cnt1, count(case when sex=(?) then 1 end )cnt2, count(case when money=(?) then 1 end )cnt3 FROM user_company where user_number=(?)";
			//조건에 맞으면 1을반환 아니면 0
		...
		//가장 count가 많은사람 10명의 결과를 닮고있음 user_number_count_desc[]]
		for(i=0;i<10;i++){
		...
		//***************************************************************************************************************
	 	...
	 	String Sql = "insert into user_personal values(0,?,?,?,?,?,0,0,?,?,?,?,0,?)";
		pstmt = cn.prepareStatement(Sql.toString());
		//pstmt.setInt(1,user_number);
		pstmt.setString(1,user_title);
		pstmt.setString(2,user_emp);
		pstmt.setString(3,user_gender);
		pstmt.setString(4,user_area);
		pstmt.setString(5,user_week);
		pstmt.setString(6,user_strong);
		pstmt.setString(7,user_foreign);
		pstmt.setString(8,user_money);
		pstmt.setString(9,user_spinner);
		pstmt.setString(10,number);
		int ok =pstmt.executeUpdate();
		
		
		String Sql_user_number_LIMIT_personal = "select user_number from user_personal order by user_number desc LIMIT 1";
	 	stmt=cn.createStatement();
		ResultSet rs_LIMIT_personal = stmt.executeQuery(Sql_user_number_LIMIT_personal);
	 	
		int LIMIT_personal =0;
		if(rs_LIMIT_personal.next()){
			LIMIT_personal = rs_LIMIT_personal.getInt("user_number");
		}
		...
		jObject.put("number",LIMIT_personal);
