...
String[] collage = request.getParameterValues("collage");
...
int a=0;
String collage_all = "";
for(a = 0; a < collage.length;a++){
	collage_all+=collage[a]+",";
}
collage_all = collage_all.substring(0,collage_all.length()-1);
String week_all = "";
...
try{
	...
	for(int i =1; i<=LIMIT;i++){
		...
		//조건에 맞으면 1을반환 아니면 0 
		String Sql_all = "SELECT user_number, count(case when hire=(?) then 1 end)cnt1, count(case when sex=(?) then 1 end )cnt2,count(case when money=(?) then 1 end )cnt3, count(case when time1=(?) then 1 end )cnt4 , count(case when time2=(?) then 1 end )cnt5 FROM user_personal where user_number=(?)";
	...
	String sql = "insert into user_company(title, hire, sex, age, collage, week, time1, time2, special, language, money, renew,count,number) values(?,?,?,?,?,?,?,?,?,?,?,?,0,?)";
	...
	pstmt2.setString(4, age);
	pstmt2.setString(5, collage_all);
	pstmt2.setString(6, week_all);
	pstmt2.setString(7, time1);
	pstmt2.setString(8, time2);
	pstmt2.setString(9, special_all);
	pstmt2.setString(10, language_all);
	pstmt2.setString(11, money);
	pstmt2.setString(12, renew);
	pstmt2.setString(13,session.getAttribute("user_number").toString());
    ...
    <jsp:param name="match" value="company"/>
