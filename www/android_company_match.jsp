...
<%	
	...
	//test link
	//http://localhost:8080/android_test02/android_company_match.jsp?best_user0=19&best_user1=2&best_user2=23&best_user3=4&best_user4=14&best_user5=16&best_user6=17&best_user7=18&best_user8=19&best_user9=20
	//--------------personal이 돌렷을떄------------------------------
	...
	try{
		...
		for(int i = 0 ; i < 10 ; i ++){
			...
			if(rs.next()){
				...
				jObject.put("user_id"+i,number);
			}else if(!rs.next()){
				jObject.put("msg_ok","false");
			}*/
			String Sql = "select a.* , b.* from user a, user_personal b where a.user_number = b.number and b.user_number=(?)";
			pstmt = cn.prepareStatement(Sql.toString());
			pstmt.setInt(1,best_user[i]);
			ResultSet rs = pstmt.executeQuery();
			...
			if(rs.next()){
				...
				jObject.put("area"+i,rs.getString("area"));
